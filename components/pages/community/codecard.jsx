...
import { getSolutionsComments,getSolutionsLikes,getSolutionsDisLikes } from "@/data/get-problems";
    ...
    return (
      <div className="flex  flex-col gap-3 max-w-[22rem] md:max-w-5xl">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="actions flex gap-2">
            <Suspense fallback={<Skeleton className="w-22" />}>
              <ActionButton variant="outline" className="rounded-lg" isLoading={likeAdding} onclick={async ()=>await addLike()}>
                {likes?.number==0?null:likes?.number }
                <ArrowBigUp />
              </ActionButton>
              <ActionButton variant="ghost" className="rounded-lg" isLoading={disLikeAdding} onclick={async ()=>await disLike()}>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm truncate">{props?.description?props.description:"user did not add context."}</p>
        </div>
        <div className="flex items-center justify-between">
